...
### 问题 6: 上下文切换进行中，Timer 2 的 1ms tick 到来会怎样？

这是一个经典的中断嵌套问题。Timer 2 每 1ms 产生一次中断（INT14），如果恰好在 `portTICK_ISR` 执行上下文切换的中间时刻到期，会发生什么？

**核心前提**：C28x 进入 ISR 时硬件自动设置 INTM=1（关全局中断），`portTICK_ISR` 内部不手动开中断，且 Tick 和 Yield 共享同一个 ISR。

**结论：不会出问题**。1ms tick 不会打断正在执行的上下文切换，只会在 IFR 中挂起，等当前 ISR 执行完 IRET 后立即再次触发。

#### 场景 A：上下文切换进行中，Timer 2 到期

```
时间线：
─────────────────────────────────────────────────────────→
   [portTICK_ISR 执行中，INTM=1]
   ↑                        ↑                    ↑
   Timer 2 触发             上下文切换            IRET 退出 ISR
   (IFR 标志挂起)          (换 TCB/SP)          INTM 恢复
                                                 ↓
                                          发现 IFR 中 Timer 2 挂起
                                          → 立即再次进入 portTICK_ISR
```

因为 INTM 已关闭，Timer 2 的中断请求只能在 IFR 中**挂起（pending）**，CPU 不响应。当前 ISR 执行完 IRET 后：

1. INTM 自动恢复
2. 硬件发现 IFR 中 Timer 2 标志位还在
3. 立刻再次进入 `portTICK_ISR`

此时上一次上下文切换已经完成，新任务的栈/寄存器已恢复。这次 ISR 属于"新的一次 tick"：保存当前任务上下文 → `xTaskIncrementTick` → 需要的话再次切换。

多执行一次 ISR 的开销在微秒级，不会导致 tick 丢失或调度错乱。

#### 场景 B：portYIELD() 触发切换的同时，Timer 2 也到期

`portYIELD()` 通过写 PIEIFR1 触发 PIE 第 1 组第 3 号中断（INT1 线），Timer 2 走 INT14（非 PIE）。两者最终都调用同一个 `portTICK_ISR`，但 ISR 内部通过 `bYield` 标志位区分：

1. 进入 ISR 时 `bYield=1`（yield 触发），同时 INT14 的 tick 标志也在 IFR 中
2. ISR 处理 yield → 跳过 `xTaskIncrementTick` → 上下文切换
3. IRET 退出
4. INT14 在 IFR 中挂起，再次进入 ISR
5. 这次 `bYield=0`，正常执行 `xTaskIncrementTick`

不会出现"丢失一次 tick"或"重复计数"的问题。这也是 FreeRTOS 在所有架构上的通用设计 — tick ISR 本身不可嵌套，上下文切换在 ISR 内部完成，pending 的下一个 tick 在切换完成后自然被响应。
